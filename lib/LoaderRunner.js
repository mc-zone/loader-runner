	...
	Object.defineProperty(obj, "request", {
		...
		set: function(value) {
			if(typeof value === "string") {
				...
				obj.path = obj.origin = splittedRequest[0];
...
function simplifiyLoaderName(loaderObject, loaderContext) {
	var name = loaderObject.origin || loaderObject.path;
	return /\//.test(name) ? path.relative(loaderContext.rootContext, name) : name;
}
...
function iteratePitchingLoaders(options, loaderContext, callback) {
	...
	loadLoader(currentLoaderObject, function(err) {
		...
		runSyncOrAsync(
			...
			function(err) {
				if(err) {
					err.from = simplifiyLoaderName(currentLoaderObject, loaderContext);
					return callback(err);
				}
...
exports.simplifiyLoaderName = simplifiyLoaderName;
